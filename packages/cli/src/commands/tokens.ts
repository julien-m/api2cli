...
import { getCredsEntry, getCredsToken, isCredsAvailable, maskToken } from "../lib/creds.js";
...
export const tokensCommand = new Command("tokens")
	...
	.action((opts) => {
		...
		for (const row of rows) {
			const display = row.token ? (opts.show ? row.token : maskToken(row.token)) : pc.dim("not set");
			console.log(`  ${pc.bold(row.name.padEnd(25))} ${pc.dim(row.entry.padEnd(28))} ${display}`);
		}
		console.log();
	});
