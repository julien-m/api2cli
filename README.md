...
```bash
# Authentication (token stored in OS Keychain via creds — no plaintext)
<app>-cli auth set              # Interactive masked prompt (or pipe a value)
...
```bash
api2cli tokens              # List entries for installed CLIs (masked)
api2cli tokens --show       # Show full tokens
creds rm global/dev/<app>   # Manually remove a token
```

`api2cli doctor` checks that `creds` is available.
